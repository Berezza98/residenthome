    ...
    _createClass(ProductVariants, [{
      ...
      value: function _updateAddToCartButton(newVariant) {
        ...
        if (!newVariant) {
          ...
          addToCartButtonElement.innerHTML = window.languages.productFormUnavailable + ' · <span class="atc-price">' + Currency.formatMoney(newVariant['price'], window.theme.moneyFormat) + '</span>';
          ...
          if (newVariant['available']) {
            ...
            addToCartButtonElement.innerHTML = window.languages.productFormAddToCart + ' · <span class="atc-price">' + Currency.formatMoney(newVariant['price'], window.theme.moneyFormat) + '</span>';
          } else {
            addToCartButtonElement.setAttribute('disabled', 'disabled');
            addToCartButtonElement.classList.add('button--disabled');
            addToCartButtonElement.classList.remove('button--primary');
            addToCartButtonElement.removeAttribute('data-action');
            addToCartButtonElement.innerHTML = window.languages.productFormSoldOut + ' · <span class="atc-price">' + Currency.formatMoney(newVariant['price'], window.theme.moneyFormat) + '</span>';
          }
      ...
      value: function _onOptionChanged(event, target) {
        ...
        // var addToCartButtonElement = this.element.querySelector('.product-form__add-button .atc-price');
        ...
        if (this.currentVariant) {
          ...
          var variantImgId = this.currentVariant.featured_media;
          var variantImg = variantImgId && document.getElementById(variantImgId.id.toString());
          if (variantImg) {
            const y = variantImg.getBoundingClientRect().top + window.scrollY;
            window.scroll({
              top: y,
              behavior: 'smooth'
            });
          ...
          var allVarSpecs = document.querySelectorAll('.dynamicSpec');
          var allDynamicPrefPrices = document.querySelectorAll('.dynamicPrefPrice');
          var varSpecs = document.querySelectorAll('.dynamicSpec[data-variant-id="' + this.currentVariant.id + '"]');
          var varDynamicPrefPrice = document.querySelectorAll('.dynamicPrefPrice[data-variant-id="' + this.currentVariant.id + '"]');
          if (allVarSpecs.length && varSpecs.length) {
            [].forEach.call(allVarSpecs, function(el) {
              el.classList.add('d-none');
            });
            [].forEach.call(varSpecs, function(el) {
              el.classList.remove('d-none');
            });
          }
          if (allDynamicPrefPrices.length && varDynamicPrefPrice.length) {
            [].forEach.call(allDynamicPrefPrices, function(el) {
              el.classList.add('d-none');
            });
            [].forEach.call(varDynamicPrefPrice, function(el) {
              el.classList.remove('d-none');
            });
          }
        }
  ...
  var ProductGallery = /*#__PURE__*/function () {
    ...
    _createClass(ProductGallery, [{
      ...
      value: function variantHasChanged(newVariant) {
        var _this = this;
        var variantColor = newVariant.option1;
        ...
        if (variantColor) {
          var variantColorReady = variantColor.toLowerCase();
          // console.log(variantColorReady);
          var allColorImages = document.querySelectorAll('.pdp-feature--filter-color-images [data-group-value]');
          var filteredColorImages = document.querySelectorAll('.pdp-feature--filter-color-images [data-group-value="' + variantColorReady + '"]');
          if (filteredColorImages.length > 0) {
            [].forEach.call(allColorImages, function(el) {
              el.classList.add('is-hidden');
            });
            [].forEach.call(filteredColorImages, function(el) {
